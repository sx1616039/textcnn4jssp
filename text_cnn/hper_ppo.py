...
from env_img3 import JobEnv
from text_cnn import Actor, Critic
...
class PPO:
    def __init__(self, j_env, memory_size=5, batch_size=32, clip_ep=0.2):
        ...
        self.actor_net = Actor(hight=self.env.job_num, width=self.env.machine_num, out_num=self.action_dim)
        self.critic_net = Critic(hight=self.env.job_num, width=self.env.machine_num)
    ...
    def test(self, data_set):
        ...
        return min(value), 30, time.time() - t0, 0


if __name__ == '__main__':
    prefix = "6-idle-img3-cnn6-0999-data_set_uncertain_time1-"
    param = [prefix, "converged_iterations", "total_time", 'min']
    path = "../data_set_uncertain_time/"
    for i in range(5):
        name = prefix + str(i)
